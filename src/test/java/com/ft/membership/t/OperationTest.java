...
public class OperationTest {
  ...
  public void log_success_with_start_params() throws Exception {
    ...
    verify(mockLogger)
        .info(
            eq(
                "operation=\"simple_success\" userId=\""
                    + userId
    ...
    verify(mockLogger)
        .info(
            eq(
                "operation=\"simple_success\" outcome=\"success\" userId=\""
                    + userId
                    + "\" y=\"that quick brown fox\" zeta=\"z\" beta=\"b\""));
  }
  ...
  public void log_success_with_key_yield() throws Exception {
    ...
    verify(mockLogger)
        .info(
            eq(
                "operation=\"simple_success\" outcome=\"success\" y=\"that quick brown fox\" email=\""
                    + email
                    + "\""));
  }
  ...
  public void log_success_with_kv_yield() throws Exception {
    ...
    verify(mockLogger)
        .info(
            eq(
                "operation=\"simple_success\" outcome=\"success\" y=\"that quick brown fox\" userEmail=\""
                    + email
                    + "\" delta=\"d\" kappa=\"k\""));
  }
  ...
  public void log_simple_failure() throws Exception {
    ...
    verify(mockLogger)
        .error(eq("operation=\"simple_failure\" outcome=\"failure\" errorMessage=\"boo hoo\""));
  }
  ...
  public void log_failure_with_quiet_exception() throws Exception {
    ...
    verify(mockLogger)
        .error(eq("operation=\"simple_failure\" outcome=\"failure\" errorMessage=\"bang!\""));
  }
  ...
  public void log_failure_with_parameters_and_exception() throws Exception {
    ...
    verify(mockLogger)
        .error(
            eq(
                "operation=\"simple_failure\" outcome=\"failure\" errorMessage=\"got a puncture\" x=101 y=\"bat\" tyre=\"right\" exception=\"java.lang.RuntimeException: bang!\""),
            eq(ex));
  }
  ...
  public void allow_null_values_for_parameters_and_yields() throws Exception {
    ...
    verify(mockLogger)
        .info(
            eq(
                "operation=\"allow_nulls\" outcome=\"success\" nullableInput=null nullableResult=null"));
  }
  ...
  public void not_allow_null_values_for_operation_keys() throws Exception {
    operation("no_null_operation_keys").with((String) null, "aValue");
  }
  ...
  public void not_allow_null_values_for_yield_keys() throws Exception {
    operation("no_null_yield_keys")
        .started(mockLogger)
        .wasSuccessful()
        .yielding((String) null, "aValue");
  }

  @Test(expected = NullPointerException.class)
  public void not_allow_null_value_for_exception() throws Exception {
    operation("no_null_yield_keys").started(mockLogger).wasFailure().throwingException(null).log();
  }
  ...
  public void log_error_if_used_in_try_with_resources_and_not_terminated() throws Exception {
    ...
    verify(mockLogger)
        .error(
            eq(
                "operation=\"try-with-resources\" outcome=\"failure\" errorMessage=\"Programmer error: operation auto-closed before wasSuccessful() or wasFailure() called.\" a=5 exception=\"java.lang.IllegalStateException: Programmer error: operation auto-closed before wasSuccessful() or wasFailure() called.\""),
  ...
  public void log_normally_if_used_in_try_with_resources_and_properly_terminated()
    ...
    verify(mockLogger).info(eq("operation=\"try-with-resources\" outcome=\"success\" a=5"));
  }
  ...
  public void testOperationMidTermYield() {
    ...
    Operation operation = operation("testOperation").started(mockLogger);

    operation.logIntermediate().yielding("action", action).log();

    operation.wasSuccessful().log();

    verify(mockLogger).info(eq("operation=\"testOperation\""));

    verify(mockLogger).info(eq("operation=\"testOperation\" action=\"testAction\""));

    verify(mockLogger).info(eq("operation=\"testOperation\" outcome=\"success\""));
  }
  ...
  public void testOperationMidTermYieldWithMapOfValues() {
    ...
    verify(mockLogger).debug(eq("operation=\"testOperation\" action=\"testAction\" numOfRows=10"));
  }
  ...
  public void documentation() throws Exception {
    ...
    class Probe {
      Flight launch() {
        if (System.currentTimeMillis() % 2 == 0) throw new LaunchException();
        else return new Flight();
      }
    ...
    Operation operation =
        Operation.operation("launch").with("probe", probe).with("target", target).started(this);

    try {
      Flight f = probe.launch();

      operation.wasSuccessful().yielding("position", f.position()).log();
